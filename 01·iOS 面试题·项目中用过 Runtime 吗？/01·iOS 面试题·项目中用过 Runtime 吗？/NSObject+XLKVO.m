...
#define XLKVOClassPrefix @"XLKVONotifying_"
...
- (void)xl_addObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath completeBlock:(XLKVOCompleteBlock)completeBlock{
    ...
    NSString *setterSelName = [self x_fetchSetterSelNameWithKey:keyPath];
    ...
    //此时还没有派生子类
    if (![currentClassName hasPrefix:XLKVOClassPrefix]) {
        //派生自己的子类
        currentClass = [self xl_fetchXLKVOClassWithOriginalClassName:currentClassName];
    ...
    singleKVODic[XLKVODic_Observer] = observer;
    singleKVODic[XLKVODic_Key] = keyPath;
    singleKVODic[XLKVODic_Block] = completeBlock;
    
    //4.2 获取关联 装有所有监听者的 数组
    NSMutableArray *KVODictArray = objc_getAssociatedObject(self, XLKVODictArrayKey);
    if (KVODictArray == nil) {
        KVODictArray =  [NSMutableArray array];
        objc_setAssociatedObject(self, XLKVODictArrayKey, KVODictArray, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
- (void)xl_removeObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath {
    ...
    for (NSDictionary *KVODict in KVODictArray) {
        if([KVODict[XLKVODic_Key] isEqualToString:keyPath]){
...
Class XLKVOClass_ClassIMP (id self, SEL cmd) {
...
/**
 获取派生的类名
 */
- (Class)xl_fetchXLKVOClassWithOriginalClassName:(NSString *)originalClassName {
    // 1.获取将要派生的KVOClass的类名
    NSString *KVOClassName = [XLKVOClassPrefix stringByAppendingString:originalClassName];
    ...
    //修改class方法的实现
    class_addMethod(KVOClass, @selector(class), (IMP)XLKVOClass_ClassIMP, type);
...
static void KVOClass_ObserveKeySetterIMP (id self, SEL cmd, id newValue) {
    ...
    NSString *getterSELName = [self x_fetchGetterSELNameWithSetterSELName:setterSELName];
    ...
    for (NSMutableDictionary *KVODict in KVODictArray) {
        if ([KVODict[XLKVODic_Key] isEqualToString:getterSELName]) {
            //3.3 回调该观察者的block
            dispatch_async(dispatch_get_main_queue(), ^{
                XLKVOCompleteBlock block = KVODict[XLKVODic_Block];
                if (block) {
                    block(KVODict[XLKVODic_Observer], getterSELName, oldValue, newValue);
                }
            });
        }
    }
